...
#same condition repeat 4 times(fw and rv considered as different condition)
...
def noiseceiling_from_z_score_betas(sub, ses):
    ...
    uniqe_video_indexes = expdesignaccess.get_session_uniqe_video_indexes(
        sub, ses)
    print("Unique video indexes: ", uniqe_video_indexes)
    print("Length of raw video indexes: ", len(uniqe_video_indexes))

    dict_betas = {} # video_index: beta pair

    for video_index in uniqe_video_indexes: #pure number str
        video_number  = int(video_index)
        video_beta_pair_fw = glmaccess.read_video_betas(
            sub, video_num=video_number, direction="fw", zscore=True)  # zscore=True zscore=True zscore=True zscore=True zscore=True zscore=True zscore=True zscore=True zscore=True zscore=True zscore=True zscore=True
        ...
        video_beta_pair_rv = glmaccess.read_video_betas(
            sub, video_num=video_number, direction="rv", zscore=True)
        
        four_times_beta_pair = np.concatenate((video_beta_pair_fw, video_beta_pair_rv), axis=0)
        print("Video beta pair shape: ", four_times_beta_pair.shape)
        # like (4, 81, 95, 101) : 4 is the number of betas, 81, 95, 101 is the shape of the image

        dict_betas[video_index] = four_times_beta_pair

    dict_var_4betas = {} # video_index: var beta pair
    for video_index in dict_betas:
        four_times_beta_pair = dict_betas[video_index]
        video_var_beta_pair = np.var(four_times_beta_pair, axis=0,ddof=1)
        dict_var_4betas[video_index] = video_var_beta_pair

    sum_var_betapairs = np.zeros_like(list(dict_var_4betas.values())[0])
    for video_index in dict_var_4betas:
        video_var_beta_pair = dict_var_4betas[video_index]
        sum_var_betapairs += video_var_beta_pair
    print("Sum var betapairs shape: ", sum_var_betapairs.shape)
    
    mean_var_betapairs = sum_var_betapairs / len(dict_var_4betas)
    ...
    save_dir = "/tank/shared/2024/visual/AOT/temp/noiseceiling_NSD_same_condition_repeated_4_times_ddof_1"
