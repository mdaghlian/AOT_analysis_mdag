...
def data_construct(sub, ses, feature, Yzscore=True, Xcentered=False):
...
def test_session_inside_general(
    ...
    model_path = f"/tank/shared/2024/visual/AOT/temp/bandedRR_split_single_feature_withSTD_session_testinside/model_sub{sub}_feature_{feature}_trainses_1_Xcentered_{Xcentered}_Yzscore_{Yzscore}_Xstd_{Xstd}_testinside.joblib"
    R2_file = f"/tank/shared/2024/visual/AOT/temp/bandedRR_split_single_feature_withSTD_session_testinside/R2_score_single_sub{sub}_feature_{feature}_trainses_1_Xcentered_{Xcentered}_Yzscore_{Yzscore}_Xstd_{Xstd}_testinside.nii.gz"
    ...
    accuracy = np.mean(
        np.array(most_similar_betas_corr) == np.arange(len(most_similar_betas_corr))
    )
    print(f"Accuracy: {accuracy}")
    # save the accuracy to a file
    with open(
        f"/tank/shared/2024/visual/AOT/temp/bandedRR_split_single_feature_withSTD_session_testinside/scores/accuracy_sub{sub}_feature_{feature}_trainses_1_Xcentered_{Xcentered}_Yzscore_{Yzscore}_Xstd_{Xstd}_testtimenorm_{test_time_normalization}_testinside.txt",
        "w",
    ) as f:
        f.write(f"Accuracy: {accuracy}\n")
        f.write(f"most_similar_betas_corr: {most_similar_betas_corr}\n")
    ...
    # plt.show()
    plt.savefig(
        f"/tank/shared/2024/visual/AOT/temp/bandedRR_split_single_feature_withSTD_session_testinside/figures/plot_sub{sub}_feature_{feature}_trainses_1_Xcentered_{Xcentered}_Yzscore_{Yzscore}_Xstd_{Xstd}_testtimenorm_{test_time_normalization}_testinside_similarity_matrix.png"
    )
...
if __name__ == "__main__":
    ...
    subs = [1, 2, 3]
    features = ["semantic", "motion32"]
    Xcentereds = [True]  # , False]
    Xstds = [True]
    test_time_norms = [True]

    for sub in subs:
        for feature in features:
            for Xcentered in Xcentereds:
                for Xstd in Xstds:
                    for test_time_norm in test_time_norms:
                        test_session_inside_general(
                            ...
                            Xstd=Xstd,
                            feature=feature,
                            test_time_normalization=test_time_norm,
                        )
